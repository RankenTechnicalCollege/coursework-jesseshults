...
function runTheRegister() {
   ...
   try {
      // Throw an error if bill is greater than cash
      if (isNaN(changeValue) || changeValue < 0) {
         throw "Cash amount doesn’t cover the bill";
      }

      // Show formatted change and calculate denomination breakdown
      warningBox.innerHTML = "";
...
function calcChange(changeValue) {
   ...
   // Determine the number of $5 bills  (fixed bug: *3 → *5)
   ...
   // Convert remaining change to cents
   ...
   let coin1Amt = changeValue; // whatever remains are pennies
